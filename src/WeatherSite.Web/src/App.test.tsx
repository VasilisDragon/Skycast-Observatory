...
describe("App", () => {
  ...
  it("shows the first-visit prompt when no cookie is present", async () => {
    ...
    expect(await screen.findByText(/First load pulls NOAA\/NWS/i)).toBeInTheDocument();
  ...
  it("saves a ZIP and renders the forecast dashboard", async () => {
    ...
    await userEvent.type(screen.getByLabelText(/ZIP code/i), "60601");
    await userEvent.click(screen.getAllByRole("button", { name: /Initialize/i })[0]);

    expect(await screen.findByText(/Hourly Motion/i)).toBeInTheDocument();
  ...
  it("switches out of cookie-check mode as soon as the saved ZIP is found", async () => {
    ...
    // Boot sequence line mentions the overview endpoint while fetching
    expect(await screen.findByText(/weather\/overview/i)).toBeInTheDocument();
